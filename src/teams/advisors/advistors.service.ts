...
export class AdvisorsService {
  ...
  async create(
    ...
    try {
      ...
      let popupImgUrl = '';
      let isActive =
        createAdvisorDto.active !== undefined ? createAdvisorDto.active : true;
  ...
  async update(
    ...
    try {
      ...
      if (imageFile) {
        ...
        if (
          existingAdvisor.image &&
          existingAdvisor.image.startsWith('/assets/')
        ) {
      ...
      if (popupImageFile) {
        ...
        if (
          existingAdvisor.popupImg &&
          existingAdvisor.popupImg.startsWith('/assets/')
        ) {
          await this.fileUploadService.deleteFile(
  ...
  async getAdvisors() {
    ...
    try {
      ...
      this.logger.warn(
        'Failed to get advisors from database, falling back to static data',
      );
    }
